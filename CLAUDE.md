...
Default bleibt **`:chain`**, bis `mix lore.eval.summary` (command-r) belegt, dass `:wahrheitsbild` die verbesserte Chain-Baseline schlägt (+ Tom-OK). Phase-C-Cutover noch in Slices (aktuell: Resümee- **und Timeline-Pfad** verdrahtet; Epos-Publish folgt).
