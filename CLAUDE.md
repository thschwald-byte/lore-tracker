...
Jeder Schritt läuft in `with_status` → eigene Fehlerklassen in `/admin/errors` (#716). Alle LLM-Schritte teilen den **einen** Backend-Slot `backend_stage2`/`model_stage2_<backend>`; pro Schritt-Familie gibt es optionale **Modell-Overrides** (#783 Phase 1): `judge_model` (beide Verify-Judge-Calls — Grounding + Attribution) und `render_model` (Prosa-Renders R_n/Ep_n), leer = Extraktor-Modell. Die Overrides wirken auf allen vier Backends (`opts[:model]` schlägt den Stage-Lookup auch im CloudHelper); ein voller Backend-Split pro Schritt ist #783 Phase 2 (erst bei belegtem Qualitätsdelta). **Stil-Flavors (#787):** die Campaign-Flavors (`base` + `summary`/`epos`) + die Überschrift-Vorgabe wirken in den **Render-Prompts** (hinter dem Verify-Gate — Stil kann keine Fakten einschleusen, das Render-Gating fängt Dazudichtung); die Extraktion ist stilfrei, die Timeline deterministisch (kein chronik-Slot). Der Stil-Editor in der CampaignLive hat Tabs Resümee/Epos mit Live-Prompt-Vorschau (`preview_prompt/2`, byte-genau dieselben Builder wie die Pipeline); Überschrift nur beim Resümee (der Epos-Kapitel-Kopf ist deterministisch, #752). Historie: Default-Flip auf Wahrheitsbild 2026-07-08 nach dem Free-Seattle-Real-Lauf; Retention: historische Chain-Events/-Artefakte bleiben lesbar (Materializer-Folds + Event-Schemas unangetastet, nur die Producer sind weg).
