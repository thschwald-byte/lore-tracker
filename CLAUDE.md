...
**Ehrliche Grenzen:** kein aktiver Liveness-Check (nur Prozess-Crashes werden erkannt); eine Discord-Gateway-Session kann pro Guild nur einem Voice-Channel gleichzeitig beitreten (s.o., seit #987 laut statt destruktiv); `VoiceSession` ist RAM-only (Worker-Neustart verliert die Session ohne Re-Attach); die Live-Presence-Anzeige (wer ist im Kanal/wer spricht) fehlt noch (#988), ebenso eine hörbare Join-Ansage als Transparenz-/Consent-Signal für alle Kanal-Teilnehmer (#989).
