...
- **Jack-eigene Fehler** (`{:jack, grund}` — etwa `kein_stand`, `blockliste_geaendert`, eine Phase ohne `fertig()`) haben keine eigene Fehlerklasse und stehen in `/admin/errors` als `other`.
...
„Prüfung“ (Stufe 3) in Fakten. Resümee, Chronik und
