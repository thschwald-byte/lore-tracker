defmodule Worker.RepoFactRenderAssignmentsTest do
  @moduledoc """
  Issue #909 (Epic #900 S5): die Render-Zuordnung Fakt → Bogen
  (`Repo.fact_render_assignments/2`) — Grundlage des Arc-strukturierten
  Recap/Epos-Prompts. Gepinnt: Label-Kette (Cluster-Map, identity-merge,
  rename-Display), kind-Durchreichung (context/rauschen), FactArcSet-Override
  > Label-Kette inkl. Merge-Redirect aufs Ziel, Orphan-Override → Label-
  Gruppe, strang-lose Fakten ohne Eintrag.
  """
  ...
  test "kinds werden durchgereicht: context + rauschen (LLM-Klassifikation und Override)" do
    ...
    a = assignments()
    assert a["f1"].kind == "context"
    assert a["f2"].kind == "rauschen"
    assert a["f3"].kind == "rauschen"
  end

  test "FactArcSet schlägt die Label-Kette: Fakt trägt Titel+kind des Ziel-Bogens" do
  ...
  test "Override auf gemergten Arc landet beim Redirect-Ziel (Ziel-Titel)" do
    seed_facts!(1, [fact("f1", "der Auftrag"), fact("f2", "die Fehde")], 100)
    arc!("arc_quelle", ["irgendwas"], 101)
    arc!("arc_ziel", ["die fehde"], 102)
    merge!("arc_quelle", "arc_ziel", 103)
    fact_arc!("f1", "arc_quelle", 104)

    a = assignments()
    assert a["f1"] == %{titel: "die Fehde", kind: "arc"}
  end

  test "Orphan-Override (Arc ohne paarenden Thread) fällt auf die Label-Gruppe zurück" do
    seed_facts!(1, [fact("f1", "der Auftrag")], 100)
    arc!("arc_a", ["der auftrag"], 101)
    arc!("arc_orphan", ["nirgends gepaart"], 102)
    fact_arc!("f1", "arc_orphan", 103)
  ...
  test "unbekannte Fakten (nicht im Kampagnen-Korpus) bleiben ohne Eintrag" do
    seed_facts!(1, [fact("f1", "der Auftrag")], 100)

    a = Repo.fact_render_assignments(@cid, [fact("f_fremd", "der Auftrag"), fact("f_leer", "")])
    # f_fremd hat ein Label → Label-Gruppe existiert (dieselbe Kette) → Eintrag;
    # f_leer bleibt draußen. Die Zuordnung hängt am Label, nicht an der Präsenz
    # im Korpus — ein frisch extrahierter Fakt gruppiert sofort.
