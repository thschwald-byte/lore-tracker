defmodule Worker.Materializer.Apply2 do
  ...
  # Issue #985 Slice 1: Discord-Guild/Voice-Channel-Config, Vorbereitung für den
  # späteren Voice-Capture-Bot (Epic #985) — HAT NOCH KEINE FUNKTIONALE
  # WIRKUNG, der Bot existiert noch nicht. 1 Row/Kampagne, Muster
  # CampaignCalendarSet (apply1): LWW via fold_meta-Sidecar, Voll-Snapshot-
  # Payload. In apply2 statt apply1 — apply1 ist am God-Module-Limit (#544).
  #
  ...
  def apply_kind("CampaignDiscordConfigSet", payload, ts, meta) do
    id = payload["campaign_id"]
    event_id = Map.get(meta, :event_id)
    ...
    cond do
      ...
      not fold_supersedes?(S.campaign_discord_configs(), id, :campaign_discord_config_set, event_id) ->
        :ok
  ...
  # Issue #985 Slice 1: defensiv statt hart validiert (Calendar.from_json-
  # Vorbild): nie crashen, kein Format-Zwang (v1 hat keine funktionale
  # Wirkung). Nur trim + non-binary → "".
