defmodule Worker.Agent.Modell do
  ...
  Der Verlauf ist eine Liste von Maps mit Atom-Schlüsseln:
    ...
    * `%{role: :assistant, content: text | nil, tool_calls: [aufruf]}`
  ...
  falls das Modell eine liefert; sie geht ins Protokoll, nicht zurück an das
  Modell. `argumente` ist `{:ok, map}` oder `{:error, roh}`, wenn das Modell
