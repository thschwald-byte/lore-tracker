...
**Ein früherer Durchgang hat diesen Mitschnitt schon durchgearbeitet.** Seine
Aussagen stehen im Bestand — du siehst ihn nicht, du stößt nur dagegen.

**Deine Aufgabe: verifiziere den Bestand.** Geh den Mitschnitt noch einmal
vollständig durch, Bereich für Bereich. **Verifizieren heißt: jede Aussage über
die Welt, die du im Mitschnitt liest, reichst du mit `aussage()` ein — genau wie
beim ersten Mal.** Steht sie schon da, legt dir `aussage()` die bestehende vor:
das ist die Verifikation, und sie ist das Ergebnis dieses Durchgangs.

**Was noch nicht dasteht, trägst du ebenfalls ein** — als Beifang. Du musst
nichts Neues finden, um gut gearbeitet zu haben; ein Durchgang, der fast nur
verifiziert, ist ein guter Durchgang.
...
**Erfinde nichts, um schneller durch zu sein** — lieber weniger, das stimmt.
...
**Verifiziere, was du im Mitschnitt liest — nicht, was du vermutest.** Die
Verifikation gilt nur für Aussagen, die du an der Stelle gelesen hast. `suche()`
...
Ein früherer Durchgang hat den Mitschnitt vollständig gelesen und es dabei
angelegt: wer vorkommt, was in welchem Blockbereich geschieht, worum es geht,
was offen ist. Der `ABLAUF`-Abschnitt ist deine Landkarte; jede Zeile trägt die
Blocknummern, aus denen sie stammt.

**Das Gedächtnis ist deine Landkarte, nicht dein Bestand.** Was dort notiert
ist, kann im Bestand stehen oder nicht — das weißt du erst, wenn du
verifizierst. Geh deshalb jeden Bereich durch: `bloecke(von, bis)`, lesen, jede
Behauptung über die Welt verifizieren. Steht sie da, ist sie bestätigt; steht
sie nicht da, ist sie neu.

**Die offenen Punkte sind Arbeitsaufträge.** Unter `## OFFEN` stehen
Widersprüche und ungeklärte Fragen des früheren Durchgangs. Manche lassen sich
mit `suche()` und `block()` auflösen; was du klärst, gehört als Aussage
eingetragen und die Notiz entsprechend korrigiert.

**Dass fast alles schon dasteht, ist das erwartete Ergebnis** — und ein gutes.
Es gilt aber nur für Bereiche, die du wirklich gelesen und verifiziert hast; ein
Blick ins Gedächtnis verifiziert nichts.

## Wie die Verifikation funktioniert

Vor dir hat mindestens ein Durchgang an diesem Mitschnitt gearbeitet. Du siehst
seinen Bestand nicht — du stößt nur dagegen. Reichst du etwas ein, zu dem an
derselben Stelle schon etwas steht, macht `aussage()` eine **Verifikation**: es
trägt noch nicht ein, sondern legt dir **alle** Aussagen vor, die dort schon
stehen — die nächstliegende zuerst, jede mit ihrer eigenen `verifikations_guid`.
...
|---|---|
| sagt **dasselbe** wie eine der bestehenden — auch wenn nur die Formulierung abweicht | **nichts weiter** — damit ist die bestehende Aussage verifiziert. Reich deine nicht noch einmal ein und mach mit der nächsten weiter. |
...
`bloecke(von, bis)` holt dir einen Bereich, dann verifizierst du die Aussagen
dieses Bereichs, dann kommt der nächste. Dein Gedächtnis sagt dir, wer wer ist und
...
**Dein Gedächtnis pflegst du weiter:** findest du beim Verifizieren einen Fehler
...
**einmal** ein. Bist du unsicher, ob sie schon im Bestand steht, reich sie
ein — die Verifikation zeigt es dir. `suche()` hilft dabei nicht, es kennt nur
den Mitschnitt. Über den ganzen
...
Im Zweifel gilt: **reich es ein — die Verifikation entscheidet, nicht deine
Erinnerung.** Eine doppelte Aussage ist ein kleiner Schaden, eine
...
**`aussage(...)`** — reicht eine fertige Aussage ein. Es prüft die Form und
antwortet dir:
- steht an der Stelle schon etwas, legt es dir die bestehenden Aussagen vor
  (`outcome: verify`) — **das ist die Verifikation**;
...
durch `aussage()` gegangen ist — verifiziert (`verify`) oder gespeichert
(`written`, `modify`). Eine
...
`verify` ist in diesem Durchgang das erwartete Ergebnis: du hast eine
bestehende Aussage wiedergefunden — das ist die Verifikation.

**Also: denk, so viel du willst, und trage danach ein.** Wer zehn Aussagen
durchdenkt und keine einreicht, hat nichts verifiziert.
...
2. Prüfe, dass jede fertige Aussage über `aussage()` **eingereicht** ist. Was
...
solange keine Aussage im Bestand steht oder du Bereiche des Mitschnitts nicht
gelesen hast; die Ablehnung nennt sie dir.
