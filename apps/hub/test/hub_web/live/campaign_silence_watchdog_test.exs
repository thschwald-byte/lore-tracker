defmodule HubWeb.CampaignLive.SilenceWatchdogTest do
  ...
  test "Streamer ohne hörbares Signal seit ≥ limit wird geflaggt" do
    ...
    assert CampaignLive.compute_silent_streamers(["a"], loud_at, @now, @limit) == ["a"]
  end

  test "Streamer mit frischem Signal wird NICHT geflaggt" do
    loud_at = %{"a" => @now - 1_000}
    assert CampaignLive.compute_silent_streamers(["a"], loud_at, @now, @limit) == []
  end
  ...
  test "Streamer ohne loud_at-Eintrag (gerade beigetreten) flaggt nicht" do
    assert CampaignLive.compute_silent_streamers(["a"], %{}, @now, @limit) == []
  end
  ...
  test "mehrere stille Streamer, Reihenfolge = Streamer-Liste" do
    ...
    assert CampaignLive.compute_silent_streamers(["x", "y", "z"], loud_at, @now, @limit) ==
             ["x", "z"]
  end

  test "leere Streamer-Liste → leeres Ergebnis" do
    assert CampaignLive.compute_silent_streamers([], %{"a" => 0}, @now, @limit) == []
  end
end
