...
| Ein Mitspieler fehlt komplett im Protokoll (Discord-Aufnahme) | Für ihn lag **keine Einwilligung** vor — seine Spur wurde bewusst verworfen (#1005) | Steht in `/admin/errors` (Stage `discord_consent`, Klasse `consent_missing`). Er muss im Voice-Kanal auf **Ich stimme zu** klicken (die Nachricht postet der Bot beim Beitreten). Einmal erteilt, gilt die Zustimmung dauerhaft — auch für künftige Aufnahmen. Seine Spur beginnt **ab dem Klick**, nicht davor: die Einwilligung wirkt nur nach vorn |
| Niemand kann zustimmen / Einwilligungs-Nachricht fehlt im Voice-Kanal (#1005) | Der Bot darf im Voice-Kanal keine Nachrichten senden, oder der Text-Chat des Voice-Kanals ist deaktiviert | Steht in `/admin/errors` als `consent_button_unavailable` (Stage `discord_consent`). Dem Bot im Voice-Kanal das Recht **Nachrichten senden** geben. Wer schon früher zugestimmt hat, wird weiter aufgezeichnet — nur neue Zustimmungen sind blockiert |
| Discord-Aufnahme lässt sich nicht starten (`:discord_unavailable`), der Bot betritt keinen Kanal (#1076) | Der Worker hat keine Gateway-Verbindung zu Discord — meist, weil er beim Start schneller war als das Netz (`:nxdomain`), seltener ein abgelehnter Token | In `/settings` → Discord-Bot steht **Gateway-Verbindung**. `kein Netz — versucht es erneut`: der Worker versucht es von allein weiter (Backoff bis 5 min), nichts zu tun. `Token von Discord abgelehnt`: neuen Token eintragen — er wirkt **ohne** Worker-Neustart, der Worker merkt die Änderung innerhalb einer Minute. `kein Token hinterlegt`: Token setzen. `verbunden` und trotzdem kein Join → andere Ursache, siehe die Zeilen darunter |
| `/lore` taucht im Discord-Server nicht auf (#1033) | Die Registrierung der Slash-Commands ist gescheitert — häufigste Ursache: dem Bot-Invite fehlt der OAuth-Scope `applications.commands` | Steht in `/admin/errors` (Stage `discord_commands`, Klasse `command_registration_failed`). Bot mit `applications.commands` im Scope neu einladen, dann den Worker neu starten (registriert wird beim Gateway-Verbindungsaufbau je Server). Die Aufnahme lässt sich derweil normal über den Web-Knopf steuern |
| `/lore start` meldet, für diesen Server sei keine Kampagne konfiguriert | Guild-/Sprachkanal-ID fehlen in den Kampagnen-Einstellungen | Im Hub in der Kampagne unter Discord beide IDs eintragen. Hängen mehrere Kampagnen am selben Server, muss die gemeinte angegeben werden: `/lore start kampagne:<Name>` |
