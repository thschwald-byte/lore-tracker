...
### Mechanik — Full-Scan, blockend (seit #793)

CI fährt **`mix credo --checks LoreTracker.Credo.Check`** über das ganze Umbrella; **jeder** Verstoß rotet den PR-Check und blockt den Merge (exit 16 bei Findings, 0 sauber). Das wurde möglich, nachdem der Bestands-Backlog auf **0** geräumt war (#789: 21 Event-Kind-Literale → `Shared.Events`-SSoT; #791: `transcribe.ex`-God-Module-Split → `Transcribe.Confidence`) — ohne Backlog gibt es keine Staleness, die ein Whole-Repo-Scan sonst mitschleppt.

**Vorgeschichte — Diff-Scope als Trittstein (#544):** Bis zum Cleanup fuhr CI `credo diff --from-git-merge-base origin/master` im Warn-Modus (`failure: ignore`) — geflaggt wurde nur, was der Branch ggü. seinem master-Abzweigpunkt NEU hinzufügt, der Backlog blockte nie (**kein alterndes Baseline-File**, SonarQube „Clean as You Code"), und der Soak konnte laufen (#557-Lesson: erst beobachten, dann blockieren — Sadowski et al. CACM 2018: blocking gates brauchen effektiv 0 % FP). Nach FP-freiem Soak + Backlog-Cleanup → Blocking-Flip (#793: `failure: ignore` raus, Command auf Full-Scan). `credo diff --from-git-merge-base origin/master` bleibt der nützliche **lokale** „was fügt mein Branch neu hinzu"-Check (s. Aufruf oben).
...
Die Heimat für semantische Code-Regeln ist `tools/credo/<check>.ex` + `apps/hub/test/credo/<check>_test.exs` (siehe `ported_checks_test.exs` als Vorlage). Der Regex-basierte `mix lore.audit` (#535) wurde nach der Credo-Migration (#544) **entfernt** — Credo (AST-Custom-Checks, seit #793 Full-Scan-blockend) ist die alleinige Heimat. Beim Hinzufügen einer neuen Code-Regel: einen Credo-Custom-Check schreiben + einen `refute_issues`-Fixture-Test, der die FP-Klasse einsperrt.
